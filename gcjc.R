	...
	control=list())
{
    ...
    if(missing(par))
    {
        ...
        par <- gcjcStruct(noise=10,bias=as.vector(rowMeans(means)), config)
        names(par) <- attr(Terms,"term.labels")
    }
    if(missing(fixed)) 
    {
        fixed <- list(
            list(noise=FALSE, coeffs=c(TRUE, TRUE), bias=FALSE),
            list(noise=TRUE, coeffs=c(TRUE, TRUE), bias=FALSE)
            )
    }    
    ...
    if(opt_method == "nlminb"){
        optRes <- nlminb(start=initpar, objective = .optfun,
            ...
            response=response, x=x, zlimit=zlimit, skeleton=new_par, fixed=fixidx, equal.noise=TRUE,
...
{
    for(i in names(x$par))
    {
        cat(paste('\nNoise ', i, ':\n', sep=''));
        print(x$par[[i]]$noise, digits=digits);
        cat(paste('\nBoundary Parameters ', i, ':\n', sep=''));
        print(c(x$par[[i]]$coeff, x$par[[i]]$bias), digits=digits);
    }
